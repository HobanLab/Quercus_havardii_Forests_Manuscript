...
setwd("C:/Users/shoban/Documents/github/Qhavardii/")
...
ade_test<-read.genepop(file=paste(subset_names[subn],"/QH_GenePop_",subset_names[subn],".gen",sep=""),ncode=3)
...
genind2genalex(genclone2genind(popr_nocl),file=paste(subset_names[subn],"/QH_clone_free_",subset_names[subn],".csv",sep=""))
...
write.csv(sm_fst_mat,file=paste(subset_names[subn],"/fst_mat.csv",sep=""))
...
Qha_gd<-read.csv("GeographicDist_SquareMatrix.csv") #make sure this is right
Qha_gd<-as.matrix(Qha_gd[,-24])
Qha_gd_E<-Qha_gd[1:10,1:10]; Qha_gd_W<-Qha_gd[11:23,11:23]
Qha_fst_mat<-read.csv(file=paste(subset_names[subn],"/fst_mat.csv",sep=""))
...
#Deprecated code- using lower triangle instead of square matrix- does work
...
library("Demerelate")
Qha_rel<-read.csv(file=paste(subset_names[subn],"/QH_REL_7Indiv_11loci_RedefinedPopsMay2019.csv",sep=""),header=T)
...
# 0.381 vs 0.1866
pdf(file=paste(subset_names[subn],"/prop_half_sib.pdf",sep="")); barplot(unlist(lapply(wang.results$Empirical_Relatedness,half_sib)),las=2,ylab="average pairwise relatedness value",col=c(rep("firebrick1",10),rep("cornflowerblue",13)),names.arg=pop_names[pop_keep]); abline(h=0.25, lty=3, col="red"); #could do violin plot...
...
#####################################						  
write.csv(file=paste(subset_names[subn],"/Qha_summ_stats.csv",sep=""), cbind(pop_names[pop_keep],Qha_N_v_MLG,num_clones,Qha_Hexp,Qha_alleles,Qha_all_rich,Qha_pwfst,wang.means))

write.csv(file=paste(subset_names[subn],"/normality_pval.csv",sep=""), cbind(as.numeric(shapiro.test(Qha_alleles)[2]), as.numeric(shapiro.test(Qha_all_rich)[2]), as.numeric(shapiro.test(Qha_Hexp)[2]), as.numeric(shapiro.test(Qha_pwfst)[2]), as.numeric(shapiro.test(wang.means)[2])))

pdf(file=paste(subset_names[subn],"/normality_plots.pdf",sep=""))
par(mfrow=c(3,2))
...
write.csv(t_test_res,file=paste(subset_names[subn],"/t_test_results.csv",sep=""))
...
summ_stats<-read.csv(file=paste(subset_names[subn],"/Qha_summ_stats.csv",sep=""))
psizes<-read.csv("pop_size_estim.csv")
boxplot(summ_stats[,8]~psizes[pop_keep,3])
anova(lm(summ_stats[,8]~psizes[pop_keep,3]))
anova(lm(summ_stats[11:23,9]~psizes[11:23,3]))
boxplot((summ_stats[11:23,9]~psizes[11:23,3]))
anova(lm(summ_stats[1:13,9]~psizes[1:13,3]))
boxplot((summ_stats[1:10,9]~psizes[pop_keep[1:10],3]))
...
library("Demerelate")
all_rel_res<-matrix(nrow=23,ncol=7)				
...
wang.results<-Demerelate(Qha_rel,object=T,value="wang")	#Wang 0.5 full sibs, 0.125 half sibs?
all_rel_res[,2]<-unlist(lapply(wang.results$Empirical_Relatedness,mean))
lxy.results<-Demerelate(Qha_rel,object=T,value="lxy") 	#Lynch Ritland, parent offspring/ full sib = 0.5, half sib - 0.25, cousin = 0.125
...
rxy.results<-Demerelate(Qha_rel,object=T,value="rxy") 	#Queller, Goodnight, parent offspring/ full sib = 0.5, half sib - 0.25
all_rel_res[,5]<-unlist(lapply(rxy.results$Empirical_Relatedness,mean))			  
morans.results<-Demerelate(Qha_rel,object=T,value="morans") 	#Morans (I think Hardy, Vekemans, I think full sib = 0.5, half sib - 0.25)
all_rel_res[,6]<-unlist(lapply(morans.results$Empirical_Relatedness,mean))		

par(mfrow=c(3,2))
barplot(unlist(lapply(wang.results$Empirical_Relatedness,half_sib))); abline(h=0.25, lty=3, col="red")
barplot(unlist(lapply(lois.results$Empirical_Relatedness,half_sib)))
...
barplot(unlist(lapply(Sxy.results$Empirical_Relatedness,half_sib)))
barplot(unlist(lapply(morans.results$Empirical_Relatedness,half_sib))); abline(h=0.25, lty=3, col="red")


boxplot(wang.results$Empirical_Relatedness,half_sib); abline(h=0.25, lty=3, col="red")
boxplot(lois.results$Empirical_Relatedness,half_sib)
boxplot(rxy.results$Empirical_Relatedness,half_sib); abline(h=0.25, lty=3, col="red")
boxplot(lxy.results$Empirical_Relatedness,half_sib); abline(h=0.25, lty=3, col="red")
boxplot(Sxy.results$Empirical_Relatedness,half_sib)
boxplot(morans.results$Empirical_Relatedness,half_sib); abline(h=0.25, lty=3, col="red")

lois.means<-unlist(lapply(lois.results$Empirical_Relatedness,mean))
c(unlist(t.test(lois.means[East_pops],lois.means[West_pops])[c(3,5)]),
	as.numeric(wilcox.test(lois.means[East_pops],lois.means[West_pops])[3]))
	
lxy.means<-unlist(lapply(lxy.results$Empirical_Relatedness,mean))
c(unlist(t.test(lxy.means[East_pops],lxy.means[West_pops])[c(3,5)]),
	as.numeric(wilcox.test(lxy.means[East_pops],lxy.means[West_pops])[3]))

Sxy.means<-unlist(lapply(Sxy.results$Empirical_Relatedness,mean))
c(unlist(t.test(Sxy.means[East_pops],Sxy.means[West_pops])[c(3,5)]),
	as.numeric(wilcox.test(Sxy.means[East_pops],Sxy.means[West_pops])[3]))
}	
