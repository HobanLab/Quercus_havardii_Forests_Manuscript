...
subset_names<-c("7indiv_7loci","7indiv_11loci","10indiv_7loci","10indiv_11loci") 
...
Qha_gd<-read.csv("QH_geog_dist_mat.csv")
Qha_gd<-as.matrix(Qha_gd[,c(2:27)])
Qha_gd_E<-Qha_gd[1:13,1:13]; Qha_gd_W<-Qha_gd[14:26,14:26]
Qha_fst_mat<-read.csv("7indiv_11loci/fst_mat.csv")
Qha_fst_mat<-as.matrix(Qha_fst_mat[,-1])
Qha_fst_mat_E<-Qha_fst_mat[1:13,1:13]; Qha_fst_mat_W<-Qha_fst_mat[14:26,14:26]

pdf(file="qhe_mantel.pdf")
plot(Qha_fst_mat[1:13,1:13]~Qha_gd[1:13,1:13])
dev.off()
summary(lm(Qha_fst_mat[1:13,1:13]~Qha_gd[1:13,1:13],na.omit=T))
pdf(file="qhw_mantel.pdf")
plot(Qha_fst_mat[14:26,14:26]~Qha_gd[14:26,14:26])
dev.off()

Qha_gd<-Qha_gd[upper.tri(Qha_gd)]; Qha_fst_mat<-Qha_fst_mat[upper.tri(Qha_fst_mat)]
...
library("Demerelate");		Qha_rel<-read.csv(paste("QH_clone_free_",subset_names[subn],"_rel.csv",sep=""))
...
# 0.381 vs 0.1866
pdf(file="prop_half_sib.pdf"); barplot(unlist(lapply(wang.results$Empirical_Relatedness,half_sib)),las=2,ylab="average pairwise relatedness value"); abline(h=0.25, lty=3, col="red"); 
