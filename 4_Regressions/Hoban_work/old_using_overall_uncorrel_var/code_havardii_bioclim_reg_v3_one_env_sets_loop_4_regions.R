...
#subtract the correlated variables- TO REDO ONCE HAVE FINAL LIST... MOVE BELOW
...
for (p in 1:4){
	...
	if (p==1) region<-"E"; if (p==2) region<-"W"; if (p==3) region<-"overall"; if (p==4) region<-"W_sub"
	...
	#identify rows and columns with significant p values 
		...
		if (p==4) row_col_signif<-which( matrix(p.adjust(reg_pval,"BH"),nrow=12,ncol=5)<0.075,arr.ind=T)
	...
	if (p!=2) {
		...
		if (p==4) par(mfrow=c(4,2),mar=c(4,4,2,2),oma=c(2,3,1,1))
